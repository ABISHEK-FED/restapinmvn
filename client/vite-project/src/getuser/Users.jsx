...
function Users() {
  ...
  // fetch users
  useEffect(() => {
    axios.get("http://localhost:8010/api/users").then((res) => {
  ...
  const delfun = async (delid) => {
    ...
    try {
      await axios.delete(`http://localhost:8010/api/delete/user/${delid}`);
  ...
  const saveUpdate = async (id) => {
    try {
      const res = await axios.put(`http://localhost:8010/api/update/user/${id}`,editData);
      setAllUsers(
        allusers.map((user) =>
